...
function InfoCard({ entry }: { entry: ImportantData }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 shadow-card">
      <p className="mb-2.5 font-semibold text-primary text-sm">{entry.label}</p>
      <div className="space-y-1">
        {Object.entries(fields).map(([k, v]) => (
          <div key={k} className="flex gap-1.5 text-xs">
            <span className="font-medium text-secondary shrink-0 min-w-[100px]">{k}:</span>
            <span className="text-primary break-words">{v}</span>
          </div>
        ))}
      </div>
    </div>
...
function Section({ title, entries }: { title: string; entries: ImportantData[] }) {
  return (
    <div>
      <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-tertiary">{title}</h2>
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <p className="text-sm text-tertiary">No important data entries yet</p>
      ) : (
