...
def test_tax_summary_stock_lt_gain(client):
    asset_resp = client.post("/assets", json=make_asset(asset_type="STOCK_IN"))
    ...
    entry = next((e for e in entries if e["asset_type"] == "STOCK_IN"), None)
    assert entry is not None
    assert entry["lt_gain"] == pytest.approx(5000.0)
    assert entry["st_gain"] == pytest.approx(0.0)
    # LTCG exemption: 5000 < 125000 → taxable = 0, tax = 0
...
def test_tax_summary_stock_st_gain(client):
    ...
    # BUY Jun 2024, SELL Sep 2024 — 92 days → ST
    client.post(f"/assets/{asset_id}/transactions", json={
    ...
    resp = client.get("/tax/summary?fy=2024-25")
    entry = next(e for e in resp.json()["entries"] if e["asset_type"] == "STOCK_IN")
...
def test_tax_summary_excludes_sells_outside_fy(client):
    ...
    # SELL in FY2023-24 — should NOT appear in FY2024-25 summary
    client.post(f"/assets/{asset_id}/transactions", json={
...
def test_tax_summary_us_stock_st_is_slab(client):
    asset_resp = client.post("/assets", json=make_asset(
        asset_type="STOCK_US", asset_class="EQUITY", identifier="AAPL"
    ))
    asset_id = asset_resp.json()["id"]

    # BUY Jun 2024, SELL Sep 2024 → ST for STOCK_US → slab rate
    client.post(f"/assets/{asset_id}/transactions", json={
    ...
    resp = client.get("/tax/summary?fy=2024-25")
    entry = next(e for e in resp.json()["entries"] if e["asset_type"] == "STOCK_US")
    assert entry["is_st_slab"] is True
    assert entry["st_tax_estimate"] is None
...
def test_tax_unrealised_with_price_cache(client, db):
    ...
    # Seed price cache directly — 1200 INR = 120000 paise
    db.add(PriceCache(
...
def test_tax_harvest_with_loss(client, db):
    ...
    # Price below cost → unrealised loss — 800 INR = 80000 paise
    db.add(PriceCache(
    ...
    assert opp["unrealised_loss"] == pytest.approx(2000.0)   # 10 × (800 - 1000)
    assert opp["unrealised_gain"] < 0
